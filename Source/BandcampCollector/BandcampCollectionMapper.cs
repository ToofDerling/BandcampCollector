...
{
    public class BandcampCollectionMapper
    {
        ...
        private void AddBandcampCollectionItems(ICollection<ParsedCollectionItem> parsedCollectionItems)
        {
            foreach (var item in parsedCollectionItems)
            {
                //https://bandcamp.com/download?from=collection&payment_id=4244706356&sig=c8789041740732a89fe4bd050a0fa1c9&sitem_id=243289570

                var id = item.sale_item_id.ToString();
                var query = HttpUtility.ParseQueryString(item.RedownloadUrl.Value);

                var bcItem = new BandcampCollectionItem
                {
                    Id = id,
